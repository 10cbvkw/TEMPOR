...
from src.models import CryoFIRE, CryoFIRE_TEMPOR
...
class Trainer:
    def __init__(self, config_dict):
        ...
        self.data = dataset.MRCData(args.particles, norm=args.norm, keepreal=True, ind=args.ind,
                                    ...
                                    invert_data=args.invert_data, seq_len=args.seq_len)
        ...
        content = torch.load('/data/yez/cryofire_yaolin/logs/cas9_1900_5_0730/weights.1000.pkl')
        self.pretrain_model.load_state_dict(content['model_state_dict'])
        ...
        shared_cnn_dict = self.pretrain_model.shared_cnn.state_dict()
        conf_regressor_dict = self.pretrain_model.conf_regressor.state_dict()
        pose_regressor_dict = self.pretrain_model.pose_regressor.state_dict()
        hypervolume_dict = self.pretrain_model.hypervolume.state_dict()

        self.model.shared_cnn.load_state_dict(shared_cnn_dict, strict=True)
        self.model.conf_regressor.load_state_dict(conf_regressor_dict, strict=True)
        self.model.pose_regressor.load_state_dict(pose_regressor_dict, strict=True)
        self.model.hypervolume.load_state_dict(hypervolume_dict, strict=True)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.start_epoch + self.num_epochs):
            ...
            for in_dict in self.data_generator:
                ...
                in_dict['index'] = in_dict['index'].reshape(in_dict['index'].shape[0] * in_dict['index'].shape[1])
                in_dict['y'] = in_dict['y'].reshape(in_dict['y'].shape[0] * in_dict['y'].shape[1], in_dict['y'].shape[2], in_dict['y'].shape[3])
                in_dict['y_real'] = in_dict['y_real'].reshape(in_dict['y_real'].shape[0] * in_dict['y_real'].shape[1], in_dict['y_real'].shape[2], in_dict['y_real'].shape[2])
    ...
    def train_step(self, in_dict, end_time):
        ...
        y_gt = in_dict['y']
        ind = in_dict['index']
        # y_gt = in_dict['y'].reshape(-1, in_dict['y'].shape[2], in_dict['y'].shape[3]) # FIXME
        # ind = in_dict['index'].reshape(-1) # FIXME
    ...
    def forward_pass(self, in_dict):
        ...
        in_dict['index'] = in_dict['index'].reshape(in_dict['index'].shape[0] * in_dict['index'].shape[1])
        in_dict['y'] = in_dict['y'].reshape(in_dict['y'].shape[0] * in_dict['y'].shape[1], in_dict['y'].shape[2], in_dict['y'].shape[3])
        in_dict['y_real'] = in_dict['y_real'].reshape(in_dict['y'].shape[0] * in_dict['y'].shape[1], in_dict['y_real'].shape[2], in_dict['y_real'].shape[2])
        '''
        ind = in_dict['index']
        batch_size = len(ind)
