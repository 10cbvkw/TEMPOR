...
class MRCData(data.Dataset):
    ...
    def __init__(self, mrcfile, norm=None, keepreal=False, invert_data=False, ind=None, window=True, datadir=None,
                 relion31=False, max_threads=16, window_r=0.7, flog=None, lazy=False, poses_gt_pkl=None, seq_len=19):
        log = flog if flog is not None else utils.log
        self.lazy = lazy
    ...
    def __getitem__(self, index):
        if self.lazy:
            ...
            for index_ in range(start1, end1, (self.N // self.seq_len)):
                images.append(self.particles[index_])
                indexs.append(index_)
                images_real.append(self.imgs[index_])
            for index_ in range(start2, end2, (self.N // self.seq_len)):
                images.append(self.particles[index_])
                indexs.append(index_)
                images_real.append(self.imgs[index_])

            images = np.stack(images)
            indexs = np.stack(indexs)
            images_real = np.stack(images_real)

            in_dict = {'y': images,
                       'index': indexs}
            if self.keepreal:
                in_dict['y_real'] = images_real
