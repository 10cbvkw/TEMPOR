...
class CryoFIRE_TEMPOR(nn.Module):
    def __init__(self, lattice, output_mask, shared_cnn_params, conf_regressor_params, hyper_volume_params,
                 no_trans=False, sym_loss=True, sym_loss_factor=4, use_gt_poses=False, seq_len=19):
        """
        lattice: Lattice
        output_mask: Mask
        shared_cnn_params: dict
        ...
        seq_len: int
        """
        super(CryoFIRE_TEMPOR, self).__init__()
        self.lattice = lattice
        self.D = lattice.D
        self.output_mask = output_mask
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(self.embed_dim, self.num_heads)
        ...
        self.rnn_h_to_g = nn.LSTM(indim, gDim, num_layers, batch_first=True, bidirectional=bidirectional)

        self.mlp_h_to_z0 = nn.Sequential(
            nn.Linear(indim, gDim),
            nn.ReLU(),
            nn.Linear(gDim, gDim),
			nn.ReLU(),
            nn.Linear(gDim, gDim),
            nn.ReLU(),
            nn.Linear(gDim, self.d_feature),
            )

        self.mlp_z_to_g = nn.Sequential(
            nn.Linear(self.d_feature, gDim),
            nn.ReLU(),
            nn.Linear(gDim, gDim),
			nn.ReLU(),
            nn.Linear(gDim, gDim),
    ...
    def cat_z(self, features, z):
        """
        features: [B, d_feature]
        z: [B, d_z]
        """
        if z is not None:
            assert features.shape[0] == z.shape[0]
            features_z = torch.cat((features, z), dim=-1)
            return features_z
        else:
            return features

    def reparameterize_z(self, temp_z):
        z_mu, z_logvar = temp_z[:, :self.d_z], temp_z[:, self.d_z:]
    ...
    def inference_DKF(self, x):
        ...
        h = x.view(batch_size, seq_len, in_dim)		
        ...
        z_list = []
        z0 = self.mlp_h_to_z0(x[:, 0, :].view(batch_size, 1, in_dim)) 
        z_list.append(z0)
    ...
    def encode(self, in_dict, pose_only):
        ...
        shape = shared_features.shape
        shared_features = shared_features.reshape(shared_features.shape[0], -1)

        batch_size_ = shared_features.shape[0]
        batch_size = batch_size_ // self.seq_len
        shared_features_ = shared_features.reshape(batch_size, self.seq_len, -1)
        shared_features = self.inference_DKF(shared_features_)
        shared_features = shared_features.reshape(batch_size_, -1)
        
        shared_features = shared_features.reshape(shape[0], shape[1], shape[2], shape[3])
        shared_features_ = shared_features_.reshape(shape[0], shape[1], shape[2], shape[3])
        ...
        if self.z_dim > 0:
            ...
            for key in conf_dict:
                latent_variables_dict[key] = conf_dict[key]
        # pose_dict = self.pose_regressor(shared_features, in_dict)
        pose_dict = self.pose_regressor(shared_features_, in_dict) # FIXME
        for key in pose_dict:
            latent_variables_dict[key] = pose_dict[key]
        return latent_variables_dict
